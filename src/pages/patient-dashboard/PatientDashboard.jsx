...
const patientNotifications = [
  {
    ...
    desc: "You achieved a 4-week streak. Keep up the great work!",
    time: "2h ago",
    unread: true,
    color: "#3ED8C8",
    bg: "rgba(62, 216, 200, 0.15)",
  },
  {
    id: 2,
    icon: "📅",
    title: "Session reminder",
    desc: "Dr. Sarah scheduled your next review for July 15.",
    time: "5h ago",
    unread: true,
    color: "#0099A6",
    bg: "rgba(0, 153, 166, 0.15)",
  },
];
...
const calendarGrid = [
  ...
  ...Array.from({ length: 31 }, (_, i) => {
    ...
    let status = "rest",
      isTherapy = false;
    if ([1, 3, 6, 8].includes(d)) {
      status = "completed";
      isTherapy = true;
    } else if (d === 9) {
      status = "today";
      isTherapy = true;
    } else if ([11, 13, 15, 17, 20, 22, 24, 27, 29, 31].includes(d)) {
      status = "upcoming";
      isTherapy = true;
    }
      ...
      >
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <div
              style={{ color: "#0C2830", fontSize: "16px", fontWeight: "700" }}
            >
              Notifications
            </div>
            <div
    ...
    {selectedPlanData && selectedPlanData.isTherapy ? (
      <>
        ...
        >
          ...
          {selectedPlanData.status === "today" && (
            <button
              ...
              style={{
                ...
                fontWeight: "700",
                cursor: "pointer",
                boxShadow: "0 4px 12px rgba(200, 112, 74, 0.25)",
          ...
          {selectedPlanData.status === "completed" && (
            <div
              style={{ color: "#7AAAB4", fontSize: "14px", lineHeight: "1.5" }}
            >
              Great job! You successfully completed all exercises for this
              session.
            </div>
          )}
          {selectedPlanData.status === "upcoming" && (
            <div
              style={{ color: "#7AAAB4", fontSize: "14px", lineHeight: "1.5" }}
            >
              This session is scheduled for later this week. Rest properly
              beforehand.
            </div>
          )}
        </div>
      ...
      >
        ...
        >
          <svg
            ...
            strokeLinejoin="round"
          >
            <path d="M21.5 2v6h-6M21.34 15.57a10 10 0 1 1-.59-8.21l-3.32 3.32"></path>
          </svg>
        </div>
        <div
          style={{
        ...
        >
          No therapy session scheduled for July {selectedPlanDate}. Enjoy your
          rest!
        </div>
      </div>
    )}
...
const PatientDashboard = ({ initialTab = "Dashboard" }) => {
  ...
  const formatLog = (log, index, allLogs) => {
    ...
    const isYesterday =
      new Date(today - 86400000).toDateString() === d.toDateString();
  ...
  const renderMinigameDetails = (game) => {
    ...
    if (game.title === "Piano Tiles") {
      stats = [
        ...
        {
          label: "Duration",
          value: `${game.duration_seconds} seconds`,
          span: 2,
        },
        ...
        {
          label: "Perfect Hits",
          value: game.perfect_hits,
          span: 1,
          color: "#4BA882",
        },
        {
          label: "Good Hits",
          value: game.good_hits,
          span: 1,
          color: "#3ED8C8",
        },
      ];
        ...
        {stats.map((s, i) => (
          <div
            ...
            style={{
              ...
              borderRadius: "16px",
              border: s.highlight
                ? "1.5px solid rgba(0, 153, 166, 0.2)"
                : "1px solid #E2E8F0",
          ...
          >
            ...
            <div
              style={{
                color: s.color || (s.highlight ? "#0099A6" : "#0C2830"),
                fontSize: s.highlight ? "28px" : "22px",
                fontFamily: "Space Mono",
                fontWeight: "700",
              }}
            >
  ...
  const selectedPlanData = calendarGrid.find(
    (day) => day.date === selectedPlanDate && day.isCurrentMonth,
  );
...
const currentPainRaw = sessionLogs.length > 0 ? sessionLogs[0].newPain : null;
  ...
  const dynamicTopStats = [
    {
      label: "RECOVERY",
      val: `Week ${Math.ceil(completedExercises / 3) || 1}`,
      sub: "of programme",
      color: "#0C2830",
    },
    {
      label: "SESSIONS",
      val: completedExercises.toString(),
      sub: "total done",
      color: "#0C2830",
    },
    {
  ...
  const dynamicChartPoints = chartPointsBase.map((log, index) => {
    const step =
      chartPointsBase.length > 1 ? 850 / (chartPointsBase.length - 1) : 0;
  ...
  const polylinePoints = dynamicChartPoints
    .map((pt) => `${pt.x},${pt.y}`)
    .join(" ");
  const polygonPoints =
    dynamicChartPoints.length > 0
      ? `50,160 ${polylinePoints} ${dynamicChartPoints[dynamicChartPoints.length - 1].x},160`
      : "50,160 50,160";

  const painTrendPercentage =
    startPain > 0 ? Math.round((painImprovement / startPain) * 100) : 0;
  const painTrendString =
    painImprovement > 0
      ? `↘ −${painTrendPercentage}%`
      : painImprovement < 0
        ? `↗ +${Math.abs(painTrendPercentage)}%`
        : `0%`;
  const painTrendColor =
    painImprovement > 0
      ? "#4BA882"
      : painImprovement < 0
        ? "#C0574C"
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    {painImprovement > 0
                      ? `↓ from ${startPain} at start`
                      : painImprovement < 0
                        ? `↑ from ${startPain} at start`
                        : `Same as start (${startPain})`}
                  </div>
                  <div
                    style={{
                      display: "flex",
                ...
                <div
                  style={{
                    flex: 1,
                    background:
                      "linear-gradient(135deg, #3ED8C8 0%, #28C0AE 100%)",
                    borderRadius: "24px",
                    padding: "24px",
                    color: "white",
                    display: "flex",
              ...
              >
                ...
                >
                  <div>
                    ...
                    <div style={{ color: "#7AAAB4", fontSize: "15px" }}>
                      Overall progress
                    </div>
                  </div>
                  <div
                ...
                >
                  {hoveredPainPoint && (
                    <div
                      style={{
                        position: "absolute",
                        left: `${(hoveredPainPoint.x / 960) * 100}%`,
                        top: `${hoveredPainPoint.y - 80}px`,
                  ...
                  >
                    ...
                    {hoveredPainPoint && (
                      <line
                        x1={hoveredPainPoint.x}
                        y1="0"
                        x2={hoveredPainPoint.x}
                        y2="160"
                        stroke="#C4E8EC"
                        strokeWidth="2"
                      />
                    ...
                    {dynamicChartPoints.length > 1 ? (
                      ...
                      <circle
                        cx={dynamicChartPoints[0].x}
                        cy={dynamicChartPoints[0].y}
                        r="5"
                        fill="#0099A6"
                      />
                    ) : null}
                    ...
                    {dynamicChartPoints.map((pt, i) => (
                      <circle
                        ...
                        cx={pt.x}
                        cy={pt.y}
                        r={hoveredPainPoint?.date === pt.date ? "9" : "6"}
                        fill="#0099A6"
                        stroke={
                          hoveredPainPoint?.date === pt.date
                            ? "#0099A6"
                            : "white"
                        }
                        strokeWidth="3"
                  ...
                  {recentSessionsDynamic.map((session, i) => (
                    <div
                      ...
                      style={{
                        ...
                        display: "flex",
                        alignItems: "center",
                        background: session.isToday
                          ? "rgba(200, 112, 74, 0.02)"
                          : "white",
                      }}
                    >
                      <div
                        style={{
                          ...
                          height: "8px",
                          borderRadius: "50%",
                          background: session.isToday ? "#0099A6" : "#E2E8F0",
                          marginRight: "16px",
                        }}
                      />
                      <div style={{ flex: 1 }}>
                        ...
                        >
                          ...
                          <span
                            style={{
                              ...
                              border: `1px solid ${session.statusColor}30`,
            ...
            <div
              ...
              style={{
                ...
                flex: "0 0 auto",
                background:
                  "linear-gradient(160deg, #EBF5F7 0%, #F0F4F8 40%, #EEF5ED 100%)",
                borderRadius: "20px",
                ...
                {isCompletedReview ? (
                  ...
                  <button
                    ...
                    style={{
                      ...
                      padding: "16px",
                      background:
                        "linear-gradient(135deg, #C2EB30 0%, #9AC404 100%)",
                      border: "none",
                      borderRadius: "16px",
                      color: "white",
                      fontSize: "16px",
              ...
              >
                <div
                  style={{
                    ...
                    height: "240px",
                    background:
                      "radial-gradient(circle, rgba(59,184,176,0.15) 0%, rgba(255,255,255,0) 70%)",
                    position: "absolute",
                    zIndex: 1,
                  }}
                />
                ...
                >
                  ...
                  >
                    Week {Math.ceil(completedExercises / 3) || 1}
                  </div>
                  <div style={{ color: "#7AAAB4", fontSize: "13px" }}>
                    {completedExercises} sessions done
                  </div>
                </div>
              </div>
              ...
              {!selectedSession ? (
                <>
                  ...
                  >
                          ...
                          style={{
                            ...
                            border:
                              activeFilter === filter
                                ...
                                : "1.5px solid #C4E8EC",
                            color:
                              activeFilter === filter ? "#0099A6" : "#3A6870",
                            fontSize: "16px",
                            fontFamily: "Space Grotesk",
                    ...
                    >
                      {filteredSessionsDynamic.length} sessions
                    </div>
                  </div>
                  ...
                  >
                    {sessionLoading ? (
                      ...
                      >
                        No sessions recorded yet. Complete a therapy session to
                    ...
                    {filteredSessionsDynamic.map((session) => (
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            ...
                            <div
                              style={{
                                background: session.statusBg,
                                border: `1.5px solid ${session.statusColor}33`,
                                color: session.statusColor,
                                padding: "4px 12px",
                                borderRadius: "100px",
                                fontSize: "13px",
                        ...
                        >
                          ...
                          >
                            <span
                              style={{ color: "#D4A843", fontSize: "20px" }}
                            >
                              {session.oldPain}
                            </span>
                            <span
                  ...
                  >
                    <div>
                      ...
                      >
                        <span
                          style={{
                            background: selectedSession.statusBg,
                            color: selectedSession.statusColor,
                            padding: "4px 12px",
                            borderRadius: "100px",
                            fontSize: "13px",
                            fontWeight: "700",
                        ...
                        <span style={{ color: "#7AAAB4", fontSize: "14px" }}>
                          {selectedSession.time} · {selectedSession.exercises}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  >
                    ...
                    >
                      <div>
                        ...
                        >
                          {activeBarIndex !== null
                            ? `${movementChart[activeBarIndex].fullName}: ${movementChart[activeBarIndex].val}%`
                            : "Hover grafik batang untuk melihat detail persentase"}
                        </div>
                      </div>
                      <div
                        style={{
                      ...
                      {movementChart.map((bar, i) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <div
                              style={{
                                color: isSelected ? "#0C2830" : "#7AAAB4",
                                fontSize: "12px",
                                fontFamily: "Space Mono",
            ...
            >
              ...
              >
                <div>
                  ...
                  >
                    Your current program schedule and progress tracking.
                  </div>
                </div>
                <BellIcon showNotif={showNotif} setShowNotif={setShowNotif} />
              </div>
                ...
                >
                  ...
                  >
                    {calendarGrid.map((day, index) => {
                      ...
                      const isSelected =
                        selectedPlanDate === day.date && day.isCurrentMonth;
                      let pillContent = null;
                      if (isCompleted) {
                        pillContent = (
                          <div
                            style={{
                              background: "rgba(75, 168, 130, 0.15)",
                              color: "#4BA882",
                              padding: "4px 6px",
                              borderRadius: "6px",
                              fontSize: "10px",
            ...
            >
              ...
              >
                {minigameHistory.length === 0 ? (
                  ...
                  >
                    No game records yet. Go play some minigames!
                  </div>
                ) : (
                    ...
                    >
                      ...
                      >
                        <button
                          ...
                          style={{
                            padding: "10px 20px",
                            background: hasChanges ? "#0099A6" : "#C4E8EC",
                            color: "white",
                            border: "none",
                            borderRadius: "10px",
                            fontSize: "15px",
                  ...
                  >
                    ...
                    >
                      ...
                      <button
                        ...
                        onMouseLeave={(e) =>
                          (e.target.style.background = "#FFE9E9")
                        }
                      >
                        Sign out of VISENSA
                      </button>
