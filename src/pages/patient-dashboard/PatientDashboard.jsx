...
const PatientDashboard = () => {
  ...
  // Penanganan User yang lebih aman agar tidak error
  ...
  const dynamicChartPoints = chartPoints;
  const polylinePoints = dynamicChartPoints.map(p => `${p.x},${p.y}`).join(" ");
  const polygonPoints = `50,160 ${polylinePoints} 900,160`;
  const painTrendBg = "rgba(75, 168, 130, 0.1)";
  const painTrendColor = "#4BA882";
  const painTrendString = "↘ −53%";
  ...
  return (
    <>
      ...
      <div style={{ width: "100vw", height: "100vh", display: "flex", background: "#F4F7F9", fontFamily: "Space Grotesk, sans-serif", padding: "24px", boxSizing: "border-box", gap: "24px", position: "relative" }}>
        ...
        {/* ============================== */}
        <div style={{ width: isSidebarOpen ? "280px" : "96px", minWidth: isSidebarOpen ? "280px" : "96px", height: "calc(100vh - 48px)", background: "#151E2C", borderRadius: "20px", display: "flex", flexDirection: "column", zIndex: 10, boxShadow: "0px 10px 40px rgba(226, 236, 249, 0.25)", transition: "all 0.4s cubic-bezier(0.2, 0.8, 0.2, 1)", flexShrink: 0, position: "relative" }}>
          <div style={{ padding: isSidebarOpen ? "30px 24px 20px" : "30px 0 20px", display: "flex", alignItems: "center", justifyContent: isSidebarOpen ? "space-between" : "center", flexShrink: 0, transition: "all 0.3s" }}>
            <div onClick={() => !isSidebarOpen && setIsSidebarOpen(true)} style={{ display: "flex", alignItems: "center", gap: "10px", cursor: isSidebarOpen ? "default" : "pointer" }} title={!isSidebarOpen ? "Expand sidebar" : undefined}>
              <img src={visensaLogo} alt="VISENSA Logo" style={{ width: "28px", height: "auto", flexShrink: 0 }} />
              <div style={{ color: "#F0FAFB", fontSize: "22px", fontWeight: "800", letterSpacing: "1px", whiteSpace: "nowrap", opacity: isSidebarOpen ? 1 : 0, width: isSidebarOpen ? "100px" : "0px", overflow: "hidden", transition: "all 0.3s" }}>VISENSA</div>
            </div>
            <div onClick={() => setIsSidebarOpen(false)} style={{ cursor: "pointer", color: "#7AAAB4", display: "flex", justifyContent: "center", alignItems: "center", opacity: isSidebarOpen ? 1 : 0, width: isSidebarOpen ? "24px" : "0px", overflow: "hidden", transition: "all 0.3s" }} title="Collapse sidebar">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "12px", flex: 1, width: "100%", padding: isSidebarOpen ? "20px 16px" : "20px 0", overflowY: "auto", overflowX: "hidden" }}>
            {["Dashboard", "Sessions", "My Plan", "Settings"].map((menu) => {
              const isActive = activeMenu === menu;
              return (
                <div key={menu} onClick={() => { setActiveMenu(menu); setSelectedSession(null); if(!isSidebarOpen) setIsSidebarOpen(true); }} style={{ width: isSidebarOpen ? "100%" : "44px", height: isSidebarOpen ? "auto" : "44px", padding: isSidebarOpen ? "14px 16px" : "0", margin: isSidebarOpen ? "0" : "0 auto", background: isActive ? "linear-gradient(135deg, #C8F135 0%, #96C000 100%)" : "transparent", borderRadius: "14px", display: "flex", alignItems: "center", justifyContent: isSidebarOpen ? "flex-start" : "center", gap: isSidebarOpen ? "14px" : "0", cursor: "pointer", boxShadow: isActive ? "0px 4px 12px rgba(31, 168, 143, 0.25)" : "none", transition: "all 0.2s", boxSizing: "border-box" }}>
                  <div style={{ display: "flex", justifyContent: "center", alignItems: "center", width: "24px", flexShrink: 0 }}>{renderIcon(menu, isActive)}</div>
                  <div style={{ color: isActive ? "#1A2332" : "#7AAAB4", fontSize: "16px", fontWeight: isActive ? "700" : "500", opacity: isSidebarOpen ? 1 : 0, width: isSidebarOpen ? "100%" : "0px", overflow: "hidden", whiteSpace: "nowrap", transition: "all 0.2s" }}>{menu}</div>
                </div>
              );
            })}
          </div>
          <div style={{ padding: isSidebarOpen ? "20px 16px 24px" : "20px 0 24px", flexShrink: 0 }}>
             <div style={{ background: "rgba(59, 184, 176, 0.06)", border: "1.5px solid rgba(59, 184, 176, 0.16)", borderRadius: "14px", padding: isSidebarOpen ? "16px" : "0px", height: isSidebarOpen ? "auto" : "0px", margin: isSidebarOpen ? "0" : "0 auto", opacity: isSidebarOpen ? 1 : 0, overflow: "hidden", transition: "all 0.3s", whiteSpace: "nowrap", width: isSidebarOpen ? "100%" : "0px" }}>
               <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px" }}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#3ED8C8" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 15l-2 5-9-5 9-5 2 5z"></path><circle cx="12" cy="8" r="6"></circle></svg><div style={{ color: "#3ED8C8", fontSize: "14px", fontWeight: "700" }}>4-week streak</div></div>
          ...
          <div className="main-content" style={{ display: "flex", flex: 1, gap: "24px", height: "calc(100vh - 48px)" }}>
            ...
            <div className="hide-scroll" style={{ flex: 1, display: "flex", flexDirection: "column", gap: "20px", overflowY: "auto", paddingRight: "10px", paddingBottom: "100px", minWidth: 0 }} onWheel={(e) => { if (mainContentRef.current) mainContentRef.current.scrollTop += e.deltaY; }} ref={mainContentRef}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
                <div><div style={{ color: "#9AABB8", fontSize: "14px", fontWeight: "500", marginBottom: "2px" }}>{greeting},</div><div style={{ color: "#1A2332", fontSize: "32px", fontWeight: "700" }}>{user?.name || "Patient"}</div></div>
              ...
              <div style={{ background: "white", borderRadius: "18px", border: "1px solid #E2E8F0", overflow: "hidden", flexShrink: 0 }}>
                <div style={{ padding: "20px 24px", borderBottom: "1px solid #E2E8F0", display: "flex", justifyContent: "space-between", alignItems: "center" }}><div style={{ color: "#0C2830", fontSize: "16px", fontWeight: "700" }}>Recent sessions</div><div style={{ color: "#7AAAB4", fontSize: "13px", fontFamily: "Space Mono" }}>last 8</div></div>
                <div>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0, flexWrap: "wrap", gap: "10px" }}>
                    <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
                      {allSessionsFilters.map((filter) => (
                        <div key={filter} onClick={() => setActiveFilter(filter)} style={{ padding: "8px 20px", background: activeFilter === filter ? "rgba(0, 153, 166, 0.08)" : "white", borderRadius: "100px", border: activeFilter === filter ? "1.5px solid #0099A6" : "1px solid #E2E8F0", color: activeFilter === filter ? "#0099A6" : "#3A6870", fontSize: "14px", fontFamily: "Space Grotesk", fontWeight: activeFilter === filter ? "600" : "400", cursor: "pointer" }}>{filter}</div>
                      ))}
                    </div>
